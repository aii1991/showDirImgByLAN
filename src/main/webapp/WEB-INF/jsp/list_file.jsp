...
<c:set var="fileBaseUrl" value="https://pcs.baidu.com/rest/2.0/pcs/file"></c:set>
<html>
<head>
                        ...
                        <div data-path="${item.path}" data-type="${item.isdir}" class="file-item">
                            <c:choose>
                                <c:when test="${item.isdir == 1}">
                                    <img src="../img/folder.png" class="img-responsive img-thumbnail img-file-thumbnail">
                                    <div class="img-desc">
                                        <span>${fileName}</span>
                                    </div>
                                </c:when>
                                <c:otherwise>
                                    <c:if test="${cfn:isImg(fileName)}">
                                        <img src="${fileBaseUrl}?method=download&access_token=${token}&path=${cfn:urlEncode(item.path)}" class="img-responsive img-thumbnail img-file-thumbnail" />
                                        <div class="img-desc">
                                            <span>${fileName}</span>
                                        </div>
                                    </c:if>
